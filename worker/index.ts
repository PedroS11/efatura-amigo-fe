...
async function enforceRateLimit(limiter: RateLimiter, ip: string): Promise<Response | null> {
  ...
  async fetch(request, env) {
    ...
    if (url.pathname.startsWith("/api/")) {
      ...
      if (isAuthSessionPath(url.pathname)) {
        const rateLimitResponse = await enforceRateLimit(env.AUTH_RATE_LIMITER, ip);
        if (rateLimitResponse !== null) {
          return rateLimitResponse;
        }
      } else {
        const rateLimitResponse = await enforceRateLimit(env.API_RATE_LIMITER, ip);
        if (rateLimitResponse !== null) {
          return rateLimitResponse;
        }
      }
      ...
      if (!baseUrl) {
        return jsonError(500, "EFATURA_API_BASE_URL is not configured on the Worker");
      }
      ...
      try {
        ...
        return jsonError(502, error instanceof Error ? error.message : "API proxy failed");
      }
    }
